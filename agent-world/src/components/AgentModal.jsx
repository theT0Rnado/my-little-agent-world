          ...
          <div style={{
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{
                ...
                color: moodColor, letterSpacing: '2px', marginBottom: '4px',
              ...
              {/* ✅ MOOD BAR — показываем процент настроения */}
              <MoodBar value={agent.moodLevel} color={moodColor} />
            </div>

            <button
                onClick={onClose}
                style={{
                  background: 'none', border: '2px solid var(--accent-pink)',
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid var(--border-color)' }}>
          {['info','chat'].map(t => (
              <button key={t} onClick={() => setTab(t)} style={{
                ...
                border: 'none', borderBottom: tab === t ? `2px solid ${moodColor}` : '2px solid transparent',
                color: tab === t ? moodColor : 'var(--text-dim)',
                fontFamily: 'var(--font-pixel)', fontSize: '7px',
                cursor: 'pointer', letterSpacing: '1px', transition: 'all 0.15s', textTransform: 'uppercase',
              }}>
                {t === 'info' ? '📋 INFO' : '💬 CHAT'}
              </button>
          ))}
...
function AgentInfo({ agent, moodColor }) {
  return (
      <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px' }}>
        ...
        {/* ✅ Статы агента */}
        <InfoSection title="STATS" color={moodColor}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
            <StatItem label="Mood" value={agent.mood || 'NEUTRAL'} color={moodColor} />
            <StatItem label="Level" value={`${agent.moodLevel ?? 0}%`} color={moodColor} />
            {agent.x !== undefined && <StatItem label="Position X" value={agent.x} color={moodColor} />}
            {agent.y !== undefined && <StatItem label="Position Y" value={agent.y} color={moodColor} />}
          </div>
        </InfoSection>
            ...
            <InfoSection title="MEMORIES" color={moodColor}>
              {agent.recollections.map((r, i) => (
                  <div key={r.id || i} style={{
                    ...
                    marginBottom: '6px', paddingLeft: '8px', borderLeft: `2px solid ${moodColor}44`,
                  }}>
                    › {r.text}
                  </div>
              ))}
        ...
        {!agent.personality && !agent.recollections?.length && !agent.plans?.length && (
            <p style={{ fontFamily: 'var(--font-mono)', fontSize: '16px', color: 'var(--text-dim)', textAlign: 'center', marginTop: '40px' }}>
              [ NO DATA AVAILABLE ]
            </p>
        )}
      </div>
...
function StatItem({ label, value, color }) {
  return (
      <div style={{
        ...
        <div style={{ fontFamily: 'var(--font-pixel)', fontSize: '6px', color: color, opacity: 0.7, marginBottom: '4px', letterSpacing: '1px' }}>
          {label.toUpperCase()}
        </div>
...
function InfoSection({ title, color, children }) {
  return (
      <div style={{ marginBottom: '20px' }}>
        <div style={{ fontFamily: 'var(--font-pixel)', fontSize: '7px', color: color, letterSpacing: '2px', marginBottom: '10px', opacity: 0.8 }}>
          ▸ {title}
        </div>
        {children}
      </div>
  ...
  return (
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0 }}>
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
          {messages.length === 0 && (
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: '16px', color: 'var(--text-dim)', textAlign: 'center', marginTop: '30px', lineHeight: 1.8 }}>
                [ TRANSMISSION CHANNEL OPEN ]<br/>
                <span style={{ fontSize: '13px', opacity: 0.5 }}>Send a message to {agent.name}</span>
              </div>
          )}
          {messages.map((msg, i) => (
              <div key={i} style={{ alignSelf: msg.role === 'user' ? 'flex-end' : 'flex-start', maxWidth: '80%', animation: 'fadeIn 0.2s ease' }}>
                {msg.role !== 'user' && (
                    <div style={{ fontFamily: 'var(--font-pixel)', fontSize: '6px', color: moodColor, marginBottom: '4px', opacity: 0.7 }}>
                      {msg.role === 'agent' ? agent.name?.toUpperCase() : 'SYSTEM'}
                    </div>
                )}
                <div style={{
                  fontFamily: 'var(--font-mono)', fontSize: '17px', lineHeight: 1.5, padding: '10px 14px',
                  background: msg.role === 'user' ? `${moodColor}22` : msg.role === 'system' ? 'rgba(255,45,120,0.1)' : 'rgba(255,255,255,0.05)',
                  border: `1px solid ${msg.role === 'user' ? `${moodColor}66` : msg.role === 'system' ? 'rgba(255,45,120,0.3)' : 'rgba(255,255,255,0.1)'}`,
          ...
          {sending && (
              <div style={{ alignSelf: 'flex-start', fontFamily: 'var(--font-pixel)', fontSize: '7px', color: moodColor, animation: 'blink 1s infinite' }}>
                {agent.name} IS PROCESSING...
              </div>
          )}
          <div ref={chatEndRef} />
        </div>
        <div style={{ borderTop: '1px solid var(--border-color)', padding: '12px 16px', display: 'flex', gap: '10px', background: 'rgba(0,0,0,0.3)' }}>
          <input
              ref={inputRef} className="px-input" value={input}
              onChange={e => setInput(e.target.value)} onKeyDown={handleKey}
              placeholder={`Message ${agent.name}...`} disabled={sending}
              style={{ flex: 1, fontSize: '16px', padding: '8px 12px' }}
          />
          <button className="px-btn" onClick={onSend} disabled={sending || !input.trim()}
                  style={{ borderColor: moodColor, color: moodColor, opacity: (sending || !input.trim()) ? 0.4 : 1 }}>
            {sending ? '...' : 'SEND'}
          </button>
        </div>
      </div>
